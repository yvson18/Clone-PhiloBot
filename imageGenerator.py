...
def geraImg(frase):
    ...
    banco = sqlite3.connect("data/image_data_ultimate.db")
    ...
    frase_de_efeito = "\" " + frase + " \"" # print(len(long_string)) nesse caso, o bot so aceita strings de no maximo 160 caracteres
...
#-------------------------------- Criando um objeto buffered reader------------------------------------
    
    binary_stream = io.BytesIO()
    img_final.save(binary_stream,"JPEG")
    binary_stream.seek(0)
    img_gerada = io.BufferedReader(binary_stream) # class _io.BuferredReader
